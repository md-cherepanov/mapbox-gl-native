...
public:
    ForwardingRendererObserver(RendererObserver& delegate_)
...
public:
    ...
    PremultipliedImage readStillImage() { return std::move(stillImage); }
...
public:
    ...
    PremultipliedImage readStillImage() { return renderer->actor().ask(&SnapshotterRenderer::readStillImage).get(); }
    ...
    void snapshot(MapSnapshotter::Callback callback) {
        ...
        map.renderStill([this, actorRef = renderStillCallback->self()](std::exception_ptr error) {
            ...
            PointForFn pointForFn{[=,
                                   center = *map.getCameraOptions().center,
