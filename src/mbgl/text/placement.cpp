...
void Placement::placeLayer(const RenderLayer& layer) {
    ...
    for (const auto& item : layer.getPlacementData()) {
        ...
        BucketPlacementParameters params{item.tile, layer.baseImpl->source, item.featureIndex, item.sortKeyRangeIndex};
...
void Placement::placeBucket(const SymbolBucket& bucket,
    ...
    if (zOrderByViewportY) {
        ...
        for (const SymbolInstance& symbol : bucket.getSymbols(params.sortKeyRangeIndex)) {
            placeSymbol(symbol);
        }
    }
